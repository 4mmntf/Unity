...
public class moveStage4 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetKey("w"))
        {
            Vector3 force = mainCamera.transform.forward * 14f;
            rb.AddForce(force,ForceMode.Force);//addforceで力を加える
        }
        //aキーで左方向に力を加える
        if (Input.GetKey("a"))
        {
            Vector3 force = -(mainCamera.transform.right) * 14f;
    ...
    void Update()
    {
        if (Input.GetKeyDown("space"))
        {
            ...
            if(canJump)
            {
                ...
                Vector3 force = new Vector3 (0.0f,800.0f,0.0f);//新しくVector3を定義する
                rb.AddForce(force,ForceMode.Force);//addforceで力を加える
                Invoke("ChangeJumpState", 6.0f);//1.5秒間ChangeJumpState関数を止める
            }
        }
        //rキーでシーンをリセット
